...
let LessonHandlerService = class LessonHandlerService {
    constructor(postrepository, mediarepository, basictools, cacheManager, lessoncategoryrepository) {
    ...
    async getallpubliclesson() {
        let _cache = await this.cacheManager.store.get(cacheKeys_entity_1._cacheKey.public_lessons);
        if (_cache) {
            return [..._cache];
        }
        ...
        await this.cacheManager.store.set(cacheKeys_entity_1._cacheKey.public_lessons, _data);
        return [..._data];
    }
    async getall_lessoncategory() {
        let _cache = await this.cacheManager.store.get(cacheKeys_entity_1._cacheKey.lessoncategory_all);
        if (_cache) {
            return [..._cache];
        }
        let _data = await this.lessoncategoryrepository.find();
        await this.cacheManager.store.set(cacheKeys_entity_1._cacheKey.lessoncategory_all, _data);
        return [..._data];
    }
    async getalllessonavatar() {
        let _cache = await this.cacheManager.store.get(cacheKeys_entity_1._cacheKey.all_lessonavatar);
        if (_cache) {
            return [..._cache];
        }
        ...
        await this.cacheManager.store.set(cacheKeys_entity_1._cacheKey.all_lessonavatar, _data);
        return [..._data];
    }
    async publishlesson(newlesson, imgarr, lesson_avatar, user) {
        let _add = await this.postrepository.save(newlesson);
        if (_add.ID) {
            await this.mediarepository.update({ media_name: lesson_avatar }, {
                parent_ID: _add.ID,
            ...
            for (let i = 0; i < imgarr.length; i++) {
                if (imgarr[i].isOrigin == true) {
                    ...
                    await this.mediarepository.update({ media_name: imgarr[i].name }, {
                    ...
                    await this.basictools.uploadimgbyurl(imgarr[i].link, config.POST_IMG_PATH, user).then(async (result) => {
                        let mediapayload = {
            ...
            await this.cacheManager.store.del(cacheKeys_entity_1._cacheKey.all_lessonavatar);
    ...
    async editlesson(ID, newlesson, imgarr, lesson_avatar, user) {
        await this.mediarepository.update({
            parent_ID: ID,
            media_category: (0, typeorm_2.In)([config.LESSON_IMG_CAT, config.LESSON_AVT_CAT])
        }, {
            media_status: "trash"
        });
        await this.mediarepository.update({
...
LessonHandlerService = __decorate([
    ...
    __param(4, (0, typeorm_1.InjectRepository)(lessoncategory_entity_1.LessonCategoryEntity)),
