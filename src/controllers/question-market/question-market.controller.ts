import { Body, Controller, ForbiddenException, PayloadTooLargeException, Post, Request, UseGuards, Query, Inject, CACHE_MANAGER, BadRequestException } from '@nestjs/common';
import { JwtService } from '@nestjs/jwt';
import { InjectRepository } from '@nestjs/typeorm';
import Joi = require('joi');
import { Observable, from } from 'rxjs';
import { BlackListTypes } from 'src/models/blacklist/blacklist.entity';
...
import { FormUploadMediaInput, MediaListEntity, UpdateFormUploadMediaInput } from 'src/models/media/media.entity';
...
import { QuestionMarketAnswerEntity, QuestionMarketAnswerInput } from 'src/models/questionmarketanswer/questionmarketanswer.entity';
...
import { UserAnswerReviewEntity, UserAnswerReviewInput } from 'src/models/useranswer_review/useranswer_review.entity';
...
export class UploadQuestionProductImageByFileController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionproductimgbyimgfile(@Request() req: any, @Query() query: FileUploadbyFormQuery): Observable<any> {
...
export class UploadQuestionProductImageByUrlController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionproductimgbyurl(@Request() req: any, @Body() body: { img_url: string }): Observable<any> {
        return from(
            this.localService.getusertempmediafiles(req.user).then(
                () => {
...
export class UploadQuestionProductAnswerImageByFileController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionproductanswerimgbyimgfile(@Request() req: any, @Query() query: FileUploadbyFormQuery): Observable<any> {
        let path_to_save = config.POST_IMG_PATH;
        if (query.file_size && query.file_size < 25000000) {
            return from(
                this.localService.getusertempmediafiles(req.user).then(
...
export class UploadQuestionProductAnswerImageByUrlController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionproductanswerimgbyurl(@Request() req: any, @Body() body: { img_url: string }): Observable<any> {
        return from(
            this.localService.getusertempmediafiles(req.user).then(
                () => {
                    return this.basictools.uploadimgbyurl(
...
export class DeleteTempQuestionProductImageController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async deletetemporaryquestionproductimg(@Request() req: JwtAuthGuardReq, @Body() body: { img_name: string }) {
        ...
        if (_file) {
            ...
            await this.cacheManager.store.del(_cacheKey.all_questionanswer_IMG)
...
export class DeleteTempQuestionUserAnswerImageController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async deletetemporaryquestionuseranswerimg(@Request() req: JwtAuthGuardReq, @Body() body: { img_name: string }) {
        let _file = await this.mediarepository.findOne({
            media_name: body.img_name,
            user_ID: req.user.user_id
        })
...
export class CreateNewQuestionProductController {
    ...
    constructor(
        ...
        private readonly questionmarketanswerRepository: Repository<QuestionMarketAnswerEntity>,
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async createnewquestionproduct(@Request() req: JwtAuthGuardReq, @Body() body: AddNewQuestionProducPostBody) {
        if (this.uploadschema.validate(body).error) {
            throw new ForbiddenException({ message: this.uploadschema.validate(body).error?.message });
        }
        ...
        await this.cacheManager.store.del(_cacheKey.all_questionanswer)
...
export class EditPrivateQuestionProductController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async editprivatequestionproduct(@Request() req: JwtAuthGuardReq, @Body() body: EditQuestionProducPostBody) {
        if (this.uploadschema.validate(body).error) {
            throw new ForbiddenException({ message: this.uploadschema.validate(body).error?.message });
        }
        ...
        await this.mediarepository.createQueryBuilder().update({
            ...
            'parent_ID = :q_ID::integer OR parent_ID = :a_ID::integer',
            {
...
export class UserRequiredReviewUpdateController {
    ...
    constructor(
        ...
        @InjectRepository(UserAnswerReviewEntity)
        private _userAnswerReviewRepository: Repository<UserAnswerReviewEntity>,
        @Inject(CACHE_MANAGER) private readonly cacheManager: AppCache
    ) { }

    @UseGuards(JwtAuthGuard)
    @Post()
    async userrequiredreviewupdate(@Request() req: JwtAuthGuardReq, @Body() body: {
        question_ID: number
    }) {

        let _allQuestions = await this._questionmarketService.getall_questionproduct();
        let _foundQuestion = _allQuestions.find(
        ...
        if (!_foundQuestion) {
            ...
            await this.cacheManager.store.del(_cacheKey.all_userAnswerReview)
            throw new ForbiddenException({ message: "[Question Market Controller] Original question not found, the state of this review will be changed to updated and fixed" })
        }
        ...
        if (!_foundOriginalAnswer) {
            ...
            await this.cacheManager.store.del(_cacheKey.all_userAnswerReview)
            throw new ForbiddenException({ message: "[Question Market Controller] Original answer not found, the state of this review will be changed to updated and fixed" })
        }
        ...
        let _result = await this._questionmarketService.updateAllPrivateReviewsInfo(
            ...
            _allQuestionAvatars.find(
                y => y.parent_ID == body.question_ID
            ),
            _allquestionIMGs.filter(
                y => y.parent_ID == body.question_ID
            ),
            _allAnswerIMGs.filter(
...
export class UserRequireMakingReviewFixedController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async userrequiremakingreviewfixed(@Request() req: JwtAuthGuardReq, @Body() body: {
        question_ID: number
    }) {
...
export class UserRequireSkipReviewUpdate {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async userrequiremakingreviewupdated(@Request() req: JwtAuthGuardReq, @Body() body: {
        question_ID: number
    }) {
...
export class UploadQuestionProductAvatarByImgFileController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionproductavatarbyimgfile(@Request() req: any, @Query() query: FileUploadbyFormQuery): Observable<any> {
        let path_to_save = config.POST_IMG_PATH;
        if (query.file_size && query.file_size < 25000000) {
            return from(
                this.localService.getusertempmediafiles(req.user).then(
...
export class UploadQuestionProductAvatarByUrlController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionproductavatarbyurl(@Request() req: any, @Body() body: { img_url: string }): Observable<any> {
        return from(
            this.localService.getusertempmediafiles(req.user).then(
                () => {
                    return this.basictools.uploadimgbyurl(
...
export class UploadQuestionMarketUserAnswerImageByFileController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionmarketuseranswerimgbyimgfile(@Request() req: any, @Query() query: FileUploadbyFormQuery): Observable<any> {
        let path_to_save = config.POST_IMG_PATH;
        if (query.file_size && query.file_size < 25000000) {
            return from(
                this.localService.getusertempmediafiles(req.user).then(
...
export class UploadQuestionMarketUserAnswerImageByUrlController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    uploadquestionmarketuseranswerimgbyurl(@Request() req: any, @Body() body: { img_url: string }): Observable<any> {
        return from(
            this.localService.getusertempmediafiles(req.user).then(
                () => {
                    return this.basictools.uploadimgbyurl(
...
export class UsersubmitQuestionAnswerController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async usersubmitquestionanswer(@Request() req: JwtAuthGuardReq, @Body() body: {
...
export class GetReviewQuestionAnswerController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async getreviewquestionanswer(@Body() body: {
        token: string
    }) {
        try {
            ...
            if (!_decode) {
                throw new BadRequestException({ message: "[Question Market Controller] Notification token is not valid" })
            }
            ...
            if (!_foundNotification) {
                throw new ForbiddenException({ message: "[Question Market Controller] Notification item not found" })
            }

            let allQuestions = await this.questionMarketService.getall_questionproduct();
            let foundQuestion = allQuestions.find(
                y => y.ID == _decode.question_ID
            )

            if (!foundQuestion) {
                throw new ForbiddenException({ message: "[Question Market Controller] Question item not found" })
            }
            ...
            if (!_foundUserAnswer) {
                ...
                throw new ForbiddenException({ message: "[Question Market Controller] User answer item not found, this notification is removed" })
            }

            if (foundQuestion.post_status != "publish") {

                let _childAnswers = [...allUserAnswer.filter(
                    y => y.parent_ID == foundQuestion.ID
                )]
                _childAnswers.forEach(
                    async (single) => {
                ...
                throw new BadRequestException({ message: "[Question Market Controller] The question of this answer has been reported as invalid, you don't have to review this answer anymore" })
            }
            ...
            let answer = {
                ..._allAnswers.find(
                    y => y.parent_ID == _decode.question_ID
                )
            }

            let _cacheMedia = await this.questionMarketService.getallquestionanswerIMG();

            let _foundMedia = _cacheMedia.filter(
                y => y.parent_ID == answer.ID
            )
...
export class UserSubmitAnswerReviewController {
    ...
    constructor(
        ...
        @InjectRepository(QuestionMarket_UserAnswerEntity)
        private questionMarketUserAnswerRepository: Repository<QuestionMarket_UserAnswerEntity>
    ) { }
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async usersubmitanswerreview(@Request() req: JwtAuthGuardReq, @Body() body: {
        correctness: boolean,
        ...
        let _foundNotis = _allNotis.find(
            item => item.ID == body.notification_ID
        )

        if (!_foundNotis) {
            throw new ForbiddenException({ message: "[Question Market Controller] This answer is already reviewed by others, you don't have to review it anymore" })
        }

        if (!_foundNotis.user_IDs.includes(req.user.user_id)) {
            throw new ForbiddenException({ message: "[Question Market Controller] You don't have the right to review this answer, please contact admin for more information" })
        }
        ...
        if (
            ...
            await this.cacheManager.store.del(_cacheKey.all_usernotifications)
            throw new ForbiddenException({ message: "[Question Market Controller] User answer not found, this notification will be deleted right away" })
        }
        ...
        let _answerer = _allUsers.find(
            y => y.ID == _UserAnswerItem.user_ID
        )
        ...
        if (!_answerer) {
            ...
            await this.cacheManager.store.del(_cacheKey.all_usernotifications)
            throw new ForbiddenException({ message: "[Question Market Controller] The answerer not found, this notification will be deleted right away" })
        }

        let _allQuestion = await this._questionmarketService.getall_questionproduct();
        let _targetQuestion = {
            ..._allQuestion.find(
                y => y.ID == _foundNotis.data.question_ID
            )
        }
        ...
        if (
            ...
            await this.cacheManager.store.del(_cacheKey.all_usernotifications)
            throw new ForbiddenException({ message: "[Question Market Controller] The original question not found, this notification will be deleted right away" })
        }
        ...
        if (!_foundOriginalAnswer) {
            ...
            await this.cacheManager.store.del(_cacheKey.all_usernotifications)
            throw new ForbiddenException({ message: "[Question Market Controller] The original answer not found, this notification will be deleted right away" })
        }
...
export class UserConfirmReviewController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async userconfirmreview(@Body() body: {
        review_ID: number,
        isLiked: boolean,
        notification_ID: number
    }) {
        ...
        let foundReview = allanswerReviews.find(
            y => y.ID == body.review_ID
        )

        let foundReviewer = allUsers.find(
            y => y.ID == foundReview.review_author
        )
        ...
        if (!foundReview || !foundReviewer) {
            ...
            await this.cacheManager.store.del(_cacheKey.all_usernotifications)
            throw new BadRequestException({ message: "[Question Market Controller] Review or reviewer doesn't exist, notification is deleted" })
        }
...
export class useranswer_reportedReviewerRequireOriginalAnswerController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async useranswer_reportedReviewerRequireOriginalAnswer(@Request() req: JwtAuthGuardReq, @Body() body: {
        user_answer_ID: number
    }) {

        let allReportLogs = await this._userauthService.getallReportLogger();
        let foundReportLog = allReportLogs.find(
            y => y.report_type == ReportLoggerTypes.questionMarketUserAnswer && y.report_sender == req.user.user_id && y.parent_ID == body.user_answer_ID
        )

        if (!foundReportLog) {
            throw new ForbiddenException({ message: "You are not allowed to see the original answer of this report" })
        }
        ...
        foundAnswer.answer_imgs = allQuestionAnswerIMGs.filter(
            y => y.media_category == config.QUESTION_PRODUCT_ANSWER_IMG_CAT && y.parent_ID == foundAnswer.ID
        )
...
export class ReportedQuestionAuthorRequiredDataController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async reportedquestion_authorrequiredata(@Request() req: JwtAuthGuardReq, @Body() body: {
        question_ID: number
    }) {
        ...
        if (!foundQuestion) {
            throw new BadRequestException({ message: "[Question Market Controller] You don't have access to this question" })
        }

        let Avatar = {
            ...[...await this._questionmarketService.getallquestionproductavatar()].find(
                y => y.parent_ID == foundQuestion.ID
            )
        }

        let imgs = [...await this._questionmarketService.getallquestionIMG()].filter(
            y => y.parent_ID == foundQuestion.ID
        )
...
export class SendPrivateMessageToQuestionAuthorController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async sendprivatemessagetoquestionauthor(@Request() req: JwtAuthGuardReq, @Body() body: {
        question_ID: number,
        ...
        if (!foundQuestion || foundQuestion.post_status != 'publish') {
            throw new ForbiddenException({ message: "Question doesn't exist anymore" })
        }
        ...
        await this.userAuthService.sendprivateMsgtoUser(
...
export class UserConfirmClearAnswerReportController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async userconfirmclearanswerreport(@Request() req: JwtAuthGuardReq, @Body() body: {
        user_answer_ID: number
    }) {
...
export class UserReportExpiredAnswerController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post()
    async userreportexpiredanswer(@Request() req: JwtAuthGuardReq, @Body() body: {
        user_answer_ID: number
    }) {
        ...
        if (!foundAnswer) {
            throw new ForbiddenException({ message: `Answer #${body.user_answer_ID} doesn't exist` })
        }

        if (
            foundAnswer.is_reported ||
            foundAnswer.is_reviewed
        ) {
            throw new ForbiddenException({ message: `Your answer is already reported or reviewed, please check the status of this answer again by refreshing page.` })
        }
        ...
        if ((now - answerDate) <= (5 * 86400 * 1000)) {
            throw new ForbiddenException({ message: `This report action is not allowed, it is now not longer than 5 days since the time this answer was sent` })
        }
        ...
        foundAnswer.waiting_reviewers.forEach(
            async y => {
                await this._userauthService.punishUserByPoint(
                    1,
                    y
                )
            }
        ...
        let _repeatAnswer = await this._questionmarketService.userSubmitNewQuestionAnswer(
            "",
            [],
...
export class UserDeleteSingleQuestionProductController {
    ...
    constructor(
        ...
        @Inject(CACHE_MANAGER) private readonly cacheManager: AppCache,
        @InjectRepository(MediaListEntity)
        private readonly mediaListRepository: Repository<MediaListEntity>
    ) { }

    @UseGuards(JwtAuthGuard)
    @Post()
    async userdeletesinglequestionproduct(@Request() req: JwtAuthGuardReq, @Body() body: {
        question_ID: number
    }) {
        ...
        if (foundAnswer) {
            ...
            let _answerMedias = [...allMedias.filter(
                y => y.parent_ID == foundAnswer.ID && [config.QUESTION_PRODUCT_ANSWER_IMG_CAT].includes(y.media_category)
            )]
