...
import {
  ...
  UserGenders,
  UserRoles,
  _UserRegisterDataInput,
} from 'src/models/userauthentication/userauth.entity';
...
import {
  NestConfig,
  SystemDefaultConfig,
} from 'src/models/config/nestconfig.interface';
...
export class userGGloginController {
  ...
  async userAuthGoogle(@Body() GGauthBody: GGLoginauthBody) {
    let _res = await this.fetchDataService.basictools.verifyGGidtoken(
      GGauthBody,
    );
...
export class NewUserRegisterController {
  ...
  async newuserregister(
    ...
    let _registeredEmail = await this.fetchDataService.cacheManager.get(
      body.user_email,
    );
    ...
    if (_registeredEmail) {
      throw new BadRequestException({
        message:
          'Email này đã được dùng để đăng ký, nếu bạn chưa nhận được Email xác nhận, vui lòng chờ 2 phút sau đó thử đăng ký lại.',
      });
    }

    await this.fetchDataService.cacheManager.set(
      body.user_email,
      body.user_email,
      {
        ttl: 120,
    ...
    let _newpass = await this.fetchDataService.basictools.genBcrypt(
      body.user_password,
    );
    ...
    let _url =
      JSON.parse(nestconfig.MAIL_DOMAIN).MY_DOMAIN +
...
export class UserLoginController {
  ...
  @Post()
  async userlogin(@Body() body: { user_email: string; user_password: string }) {
    let _allusers = await this.fetchDataService.getallusers();
    ...
    let _matched = await this.fetchDataService.basictools.matchbcrypt(
      body.user_password,
      _hashedpw,
    );
...
export class ReportUserAnswerReviewInvalidControllerInvalidController {
  ...
  @Post()
  async report_user_answer_review_invalid(
    ...
    let allUserAnswerReviews =
      await this.questionmarketService.getAllUserAnswerReviews();
    ...
    if (
      ...
      throw new ForbiddenException({
        message:
          '[User Authentication Controller] This review is not available anymore',
      });
    }
    ...
    if (foundUserAnswerReview.answerer_ID != req.user.user_id) {
      throw new ForbiddenException({
        message:
          '[User Authentication Controller] You are not allowed to access this answer',
      });
    }
    ...
    let foundUserAnswerReviewNoti = allNotis.find(
      (y) =>
        y.type == UserNotification_Types.answer_isReviewed &&
...
export class ConfirmReportInvalidAnswerReviewController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post()
  async confirm_report_invalid_answer_review(
    @Request() req: JwtAuthGuardReq,
    @Body()
    body: {
      notification_ID: number;
    ...
    if (!foundAnswerReview.report_controllers.includes(req.user.user_id)) {
      throw new BadRequestException({
        message:
          "[User Authentication Controller] You don't have the permission to report this review'",
      });
    }
    ...
    if (!foundAnswerReview || foundAnswerReview.is_reported == false) {
      ...
      throw new BadRequestException({
        message:
          '[User Authentication Controller] This report is not available anymore',
      });
    }
    ...
    if (!reporter || !reviewer) {
      throw new ForbiddenException({
        message:
          "[User Authentication Controller] Reporter's or reviewer's account is deleted",
      });
    }
...
export class ReportUserAnswerReviewInvalidController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post()
  async report_user_answer_invalid(
    @Request() req: JwtAuthGuardReq,
    @Body()
    body: {
      notification_ID: number;
    ...
    if (!foundUserAnswer || foundUserAnswer.is_reviewed == true) {
      throw new ForbiddenException({
        message:
          '[User Authentication Controller] This answer is not available anymore',
      });
    }
    ...
    let foundUserAnswerNoti = allNotis.find(
      (y) =>
        y.type == UserNotification_Types.user_answer &&
...
export class ConfirmReportInvalidAnswerController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post()
  async confirm_report_invalid_answer(
    @Request() req: JwtAuthGuardReq,
    @Body()
    body: {
      notification_ID: number;
    ...
    if (!foundAnswer.report_controllers.includes(req.user.user_id)) {
      throw new BadRequestException({
        message:
          "[User Authentication Controller] You don't have the permission to report this answer'",
      });
    }
    ...
    if (!reporter || !answerer) {
      throw new ForbiddenException({
        message:
          "[User Authentication Controller] Reporter's or answerer's account is deleted",
      });
    }
...
export class ReportInvalidQuestionController {
  ...
  async report_invalid_question(
    ...
    if (!foundQuestion || foundQuestion.post_status != 'publish') {
      throw new ForbiddenException({
        message:
          '[User Authentication Controller] This question is not available anymore',
      });
    }
...
export class ConfirmReportInvalidQuestionController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post()
  async confirm_report_invalid_question(
    @Request() req: JwtAuthGuardReq,
    @Body()
    body: {
      notification_ID: number;
...
export class UserBlockEmailFromPrivateMessageController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post()
  async userblockemailfromprivatemessage(
    @Request() req: JwtAuthGuardReq,
    @Body()
    body: {
      msg_ID: number;
    ...
    let foundUserBlacklist = allBlacklist.find(
      (y) =>
        y.type == BlackListTypes.userPrivateMessage &&
    ...
    if (!foundUserBlacklist) {
      ...
      foundUserBlacklist = await this.fetchDataService.blackListRepository.save(
        newBlacklist,
      );
    }
...
export class UserBuyASHOPitemController {
  ...
  async userbuyashopitem(
    ...
    let itemPrice = await this.fetchDataService.getShopItemPricefromCode(
      body.item_code,
    );
...
export class UploadQuestionProductAvatarByImgFileController {
  ...
  uploaduseravatarbyimgfile(
    ...
    if (query.file_size && query.file_size < 25000000) {
      return from(
        this.fetchDataService.basictools
...
export class UserUpdateProfileController {
  ...
  async userupdateprofile(
    ...
    let matched = await this.fetchDataService.basictools.matchbcrypt(
      body.user_password,
